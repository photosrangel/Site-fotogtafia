...
const SITE_URL = (Deno.env.get('SITE_URL') || 'https://photosrangel.pt').replace(/\/$/, '')
...
async function sendEmail({ to, subject, html, text }: { to: string; subject: string; html: string; text: string }) {
  ...
  if (!response.ok) throw new Error(data?.message || data?.error || `Resend respondeu ${response.status}`)
  return data
}
...
async function loadEnsaio(id: string) {
  const { data, error } = await admin.from('ensaios').select('*').eq('id', id).single()
  if (error || !data) throw new Error('Ensaio não encontrado.')
  return data
}

async function selectedCount(ensaioId: string) {
  const { count } = await admin.from('fotos').select('id', { count: 'exact', head: true })
    .eq('ensaio_id', ensaioId).eq('tipo', 'prova').eq('selecionada', true)
  return count || 0
}

async function finalCount(ensaioId: string) {
  const { count } = await admin.from('fotos').select('id', { count: 'exact', head: true })
    .eq('ensaio_id', ensaioId).eq('tipo', 'final')
  return count || 0
}

async function notifySelection(ensaio: any, codigo: string) {
  if (!codigo || codigo !== ensaio.codigo_acesso) return json({ error: 'Acesso inválido.' }, 401)
  ...
  const { count: loggedSelection } = await admin.from('admin_activity').select('id', { count: 'exact', head: true })
    .eq('activity_type', 'client_selection_completed').eq('entity_id', String(ensaio.id))
  if (!loggedSelection) {
    await admin.from('admin_activity').insert({
      activity_type: 'client_selection_completed',
      title: `Seleção finalizada — ${ensaio.titulo || 'Ensaio'}`,
      detail: `${count} fotografia(s) selecionada(s) pela cliente.`,
      entity_type: 'ensaio', entity_id: String(ensaio.id), severity: 'success',
    })
  }
    ...
    if (!ensaio.cliente_email) {
      ...
      try {
        const html = emailShell(`
          ...
          <p>Obrigado pela confiança.</p>${button('Acessar área do cliente', areaCliente)}`)
        await sendEmail({
...
async function publishFinal(req: Request, ensaio: any) {
  ...
  const publishableStatuses = ['preparando', 'aguardando_selecao', 'selecao_finalizada', 'selecionado', 'em_edicao', 'fotos_disponiveis', 'entregue']
  if (!publishableStatuses.includes(ensaio.status || 'preparando')) {
    return json({ error: 'Este ensaio não está disponível para publicação.' }, 409)
  }

  // Guardamos o estado anterior para que uma falha no e-mail não avance nem
  // altere o fluxo escolhido (com seleção ou entrega direta).
  const previousStatus = ensaio.status || 'preparando'

  if (ensaio.email_entrega_cliente_enviado_em && ensaio.expires_at) {
    return json({
      ok: true,
      email_sent: true,
  ...
  if (!ensaio.cliente_email) {
    const { error: resetError } = await admin.from('ensaios').update({
      status: previousStatus,
      delivered_at: null,
      expires_at: null,
      email_entrega_cliente_enviado_em: null,
      expired_at: null,
    ...
    return json({
      ...
      message: 'Entrega não concluída: o ensaio não possui e-mail da cliente. Adicione o e-mail e tente novamente.',
      ensaio: refreshed,
    })
  }
  ...
  try {
    ...
    const expiryDays = Math.max(Number(ensaio.expiry_days) || 30, 1)
    const expiresAt = new Date(sentAt.getTime() + expiryDays * 86400000)
    const { error } = await admin.from('ensaios').update({
      status: 'fotos_disponiveis',
      publicado_em: ensaio.publicado_em || sentAt.toISOString(),
      delivered_at: sentAt.toISOString(),
      expires_at: expiresAt.toISOString(),
      email_entrega_cliente_enviado_em: sentAt.toISOString(),
      expired_at: null,
      deletion_scheduled_at: null,
    }).eq('id', ensaio.id)
    if (error) throw new Error(`O e-mail foi enviado, mas não foi possível confirmar a entrega: ${error.message}`)
    const refreshed = await loadEnsaio(ensaio.id)
    return json({
      ok: true,
      email_sent: true,
      delivery_confirmed: true,
      message: `Fotos publicadas, cliente notificada e prazo de ${expiryDays} dias iniciado.`,
      ensaio: refreshed,
    })
  } catch (error) {
    console.error('Entrega não confirmada:', error)
    const { error: resetError } = await admin.from('ensaios').update({
      status: previousStatus,
      delivered_at: null,
      expires_at: null,
    ...
    if (resetError) console.error('Falha ao restaurar estado não entregue:', resetError)
    const refreshed = await loadEnsaio(ensaio.id)
    return json({
      ok: true,
      email_sent: false,
      delivery_confirmed: false,
      message: `Entrega não confirmada. O prazo não começou: ${error?.message || 'falha no envio do e-mail.'}`,
      ensaio: refreshed,
    })
  }
}

async function sendManualSelectionReminder(req: Request, ensaio: any) {
  const user = await requireAdmin(req)
  if (!user) return json({ error: 'Acesso administrativo necessário.' }, 401)
  if (ensaio.status !== 'aguardando_selecao') return json({ error: 'Este ensaio não está aguardando a seleção da cliente.' }, 409)
  if (!ensaio.cliente_email) return json({ error: 'Adicione o e-mail da cliente antes de enviar o lembrete.' }, 409)

  const { count: accessCount } = await admin.from('admin_activity').select('id', { head: true, count: 'exact' })
    .eq('activity_type', 'client_gallery_accessed').eq('entity_id', String(ensaio.id))
  const nome = firstName(ensaio.cliente_nome)
  const areaCliente = `${SITE_URL}/area-cliente`
  const message = accessCount
    ? 'Vimos que você já deu uma olhada nas fotografias. Quando quiser finalizar a escolha, é só voltar à sua área privada.'
    : 'Suas fotografias estão esperando por você na área privada.'
  const html = emailShell(`
    <p style="margin-top:0;color:#f0ede6;font-family:Georgia,serif;font-size:27px;line-height:1.25;">Olá, ${escapeHtml(nome)}.</p>
    <p>${escapeHtml(message)}</p>
    <p>Ensaio: <strong style="color:#f0ede6;">${escapeHtml(ensaio.titulo || 'Ensaio')}</strong>.</p>
    ${button('Continuar seleção', areaCliente)}
    <p>Se tiver alguma dúvida sobre a escolha, pode entrar em contato conosco.</p>`)
  await sendEmail({
    to: ensaio.cliente_email,
    subject: 'Um lembrete sobre a seleção das suas fotografias',
    html,
    text: `Olá, ${nome}. ${message} Acesse ${areaCliente}`,
  })
  await admin.from('admin_activity').insert({
    activity_type: 'selection_reminder_manual',
    title: `Lembrete manual enviado — ${ensaio.titulo || 'Ensaio'}`,
    detail: `Lembrete enviado à cliente${accessCount ? ' com tom suave, pois ela já acessou a galeria' : ''}.`,
    entity_type: 'ensaio', entity_id: String(ensaio.id), severity: 'info',
  })
  return json({ ok: true, email_sent: true, message: 'Lembrete enviado para a cliente.' })
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'selection_finalized') return await notifySelection(ensaio, body?.codigo)
    if (action === 'publish_final') return await publishFinal(req, ensaio)
    if (action === 'selection_reminder_manual') return await sendManualSelectionReminder(req, ensaio)
