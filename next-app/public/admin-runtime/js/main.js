...
const PUBLIC_DESIGN_DEFAULTS = {
  ...
  client_border:'fine',
  client_access_image:'/admin-runtime/images/retrato-01.jpg',
  client_focus_x:50,
  client_focus_y:50,
...
function resolvePublishedAssetUrl(value){
  ...
  if(raw.startsWith('/admin-runtime/')) return raw;
  if(raw.startsWith('legacy/')) return `/admin-runtime/${raw.slice('legacy/'.length)}`;
  if(/^\.{0,2}\/images\//i.test(raw)) return `/admin-runtime/${raw.replace(/^\.{0,2}\//,'')}`;
  if(raw.startsWith('/images/')) return `/admin-runtime${raw}`;
  if(raw.startsWith('images/')) return `/admin-runtime/${raw}`;
