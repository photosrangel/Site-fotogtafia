...
export function AdminDashboard(){
  ...
  useEffect(()=>{if(!session)return;client().from('site_content').select('content').eq('slug','admin').eq('section_key','interface').maybeSingle().then(({data})=>{if(data?.content)setUi(normalizeAdminUi(data.content))});const timer=setTimeout(loadDashboard,500);const channel=client().channel('admin-next-dashboard').on('postgres_changes',{event:'*',schema:'public',table:'galleries'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'categories'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'gallery_photos'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'mensagens'},loadDashboard).subscribe();return()=>{clearTimeout(timer);client().removeChannel(channel)}},[session,loadDashboard]);
