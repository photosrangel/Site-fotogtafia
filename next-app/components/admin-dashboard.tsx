...
export function AdminDashboard(){
  ...
  useEffect(()=>{if(!session)return;client().from('site_content').select('content').eq('slug','admin').eq('section_key','interface').maybeSingle().then(({data})=>{if(data?.content)setUi(normalizeAdminUi(data.content))});const timer=setTimeout(loadDashboard,500);const channel=client().channel('admin-next-dashboard').on('postgres_changes',{event:'*',schema:'public',table:'galleries'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'categories'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'gallery_photos'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'mensagens'},loadDashboard).on('postgres_changes',{event:'*',schema:'public',table:'admin_activity'},loadDashboard).subscribe();return()=>{clearTimeout(timer);client().removeChannel(channel)}},[session,loadDashboard]);
  ...
  return <AdminSectionShell active="/admin-react" email={session.user.email||''} eyebrow={ui.dashboard_eyebrow} title={ui.dashboard_title} menuOpen={menuOpen} setMenuOpen={setMenuOpen} onLogout={logout}>{dashboardMessage&&<div className="flash erro">{dashboardMessage}</div>}<section className="admin-view"><div className="dashboard-quick-actions"><button className="btn btn-accent" onClick={()=>openSection('/admin-react/galerias')}>+ {ui.gallery_card_button}</button><button className="btn" onClick={()=>openSection('/admin-react/ensaios')}>+ Novo ensaio</button><button className="btn" onClick={()=>openSection('/admin-react/mensagens')}>Ver mensagens</button><button className="btn" onClick={()=>openSection('/admin-react/design')}>Editar site</button></div><div className="stats-grid"><article className="stat-card"><span>Galerias</span><strong>{loadingDashboard?'—':stats.galleries}</strong></article><article className="stat-card"><span>Publicadas</span><strong>{loadingDashboard?'—':stats.published}</strong></article><article className="stat-card"><span>Categorias</span><strong>{loadingDashboard?'—':stats.categories}</strong></article><article className="stat-card"><span>Fotos públicas</span><strong>{loadingDashboard?'—':stats.photos}</strong></article><article className="stat-card"><span>Mensagens</span><strong style={stats.messages?{color:'var(--accent)'}:undefined}>{loadingDashboard?'—':stats.messages}</strong></article></div><article className="panel dashboard-activity-panel"><div className="panel-head"><h2>Atividade recente</h2><div className="admin-native-refresh"><button className="small-btn" onClick={loadDashboard} disabled={loadingDashboard}>Atualizar</button></div></div><div className="dashboard-activity-list">{loadingDashboard&&<p className="panel-copy">Carregando atividades…</p>}{!loadingDashboard&&!activities.length&&<p className="panel-copy">Nenhuma atividade registrada ainda.</p>}{!loadingDashboard&&activities.map((item,index)=><div className={`dashboard-activity-item is-${item.severity||'info'}`} key={item.id||index}><span className="dashboard-activity-dot"></span><div><strong>{item.title||'Atividade'}</strong>{item.detail&&<small>{item.detail}</small>}</div><time>{formatDate(item.created_at)}</time></div>)}</div></article></section></AdminSectionShell>;
}
