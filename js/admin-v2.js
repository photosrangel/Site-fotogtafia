...
const ADMIN_ID='e0a315bb-3614-4dbb-b020-3e8175a67e8a',BUCKET='site-gallery';
const supabase=createClient(SUPABASE_URL,SUPABASE_ANON_KEY),$=id=>document.getElementById(id);let currentGallery=null,categoriesCache=[],galleriesCache=[];
const slugify=v=>String(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
function msg(el,t,c=''){el.textContent=t||'';el.className=c?`msg ${c}`:'msg'}function flash(t,c=''){const e=$('flash');e.textContent=t;e.className=`flash ${c}`;e.hidden=false;clearTimeout(flash.timer);flash.timer=setTimeout(()=>e.hidden=true,3500)}
function setView(v){document.querySelectorAll('.admin-view').forEach(e=>e.hidden=e.id!==`view-${v}`);document.querySelectorAll('.sidebar-link[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===v));const l={dashboard:['Painel','Dashboard'],galleries:['Conteúdo','Galerias'],categories:['Organização','Categorias'],settings:['Site','Configurações']}[v];$('view-eyebrow').textContent=l[0];$('view-title').textContent=l[1];if(v==='dashboard')loadDashboard();if(v==='galleries')loadGalleries();if(v==='categories')loadCategories();if(v==='settings')loadSettings()}
async function requireAdmin(){const{data:{session}}=await supabase.auth.getSession();if(!session){$('login-screen').hidden=false;$('app').hidden=true;return false}if(session.user.id!==ADMIN_ID){await supabase.auth.signOut();$('login-screen').hidden=false;$('app').hidden=true;msg($('login-msg'),'Este usuário não possui permissão de administrador.','erro');return false}$('user-email').textContent=session.user.email||'';$('login-screen').hidden=true;$('app').hidden=false;setView('dashboard');return true}
$('login-form').addEventListener('submit',async e=>{e.preventDefault();msg($('login-msg'),'Entrando...');const{error}=await supabase.auth.signInWithPassword({email:$('login-email').value.trim(),password:$('login-password').value});if(error){msg($('login-msg'),'E-mail ou senha incorretos.','erro');return}await requireAdmin()});$('logout-btn').addEventListener('click',async()=>{await supabase.auth.signOut();location.reload()});document.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>setView(b.dataset.view)));$('dashboard-new-gallery').addEventListener('click',()=>{setView('galleries');openGalleryForm()});$('new-gallery-btn').addEventListener('click',openGalleryForm);$('close-gallery-form').addEventListener('click',closeGalleryForm);$('cancel-gallery-form').addEventListener('click',closeGalleryForm);$('gallery-title').addEventListener('input',()=>{if(!$('gallery-id').value)$('gallery-slug').value=slugify($('gallery-title').value)});$('gallery-form').addEventListener('submit',saveGallery);$('category-form').addEventListener('submit',saveCategory);$('cancel-category').addEventListener('click',resetCategoryForm);$('settings-form').addEventListener('submit',saveSettings);$('photo-upload').addEventListener('change',async e=>{const f=[...e.target.files];if(f.length&&currentGallery)await uploadPhotos(f,currentGallery);e.target.value=''});document.querySelectorAll('[data-close-modal]').forEach(e=>e.addEventListener('click',closeGalleryModal));
async function loadDashboard(){const[a,b,c]=await Promise.all([supabase.from('galleries').select('id,published'),supabase.from('categories').select('id'),supabase.from('gallery_photos').select('id,published')]);const g=a.data||[],k=b.data||[],p=c.data||[];$('stat-galleries').textContent=g.length;$('stat-published').textContent=g.filter(x=>x.published).length;$('stat-categories').textContent=k.length;$('stat-photos').textContent=p.filter(x=>x.published).length}
async function loadCategories(){const{data,error}=await supabase.from('categories').select('*').order('sort_order').order('name');if(error){flash(`Erro ao carregar categorias: ${error.message}`,'erro');return}categoriesCache=data||[];renderCategorySelect();renderCategories()}
function renderCategorySelect(){const s=$('gallery-category'),cur=s.value;s.innerHTML='<option value="">Sem categoria</option>'+categoriesCache.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');s.value=cur}
function renderCategories(){const c=$('categories-list');if(!categoriesCache.length){c.innerHTML='<p class="panel-copy">Nenhuma categoria cadastrada.</p>';return}c.innerHTML=categoriesCache.map(x=>`<div class="category-row"><div class="category-name">${esc(x.name)}</div><div class="category-slug">${esc(x.slug)}</div><div class="category-order">${x.sort_order}</div><div class="card-actions"><button class="small-btn" data-edit-category="${x.id}">Editar</button><button class="small-btn" data-delete-category="${x.id}">Excluir</button></div></div>`).join('');c.querySelectorAll('[data-edit-category]').forEach(b=>b.addEventListener('click',()=>editCategory(b.dataset.editCategory)));c.querySelectorAll('[data-delete-category]').forEach(b=>b.addEventListener('click',()=>deleteCategory(b.dataset.deleteCategory)))}
async function saveCategory(e){e.preventDefault();const id=$('category-id').value,p={name:$('category-name').value.trim(),slug:slugify($('category-slug').value.trim()),sort_order:Number($('category-order').value)||0,published:true};const r=id?await supabase.from('categories').update(p).eq('id',id):await supabase.from('categories').insert(p);if(r.error){msg($('category-msg'),r.error.message,'erro');return}msg($('category-msg'),'Categoria salva.','sucesso');resetCategoryForm();await loadCategories();await loadDashboard()}
...
function renderGalleries() {
  ...
  // Garante que a lista visual respeite a ordem atual
  galleriesCache.sort((a, b) => {
    const ordemA = Number(a.sort_order) || 0;
    const ordemB = Number(b.sort_order) || 0;
  ...
  c.innerHTML = galleriesCache.map((g, index) => `
    <article
  ...
  c.querySelectorAll('.gallery-admin-card').forEach(card => {
    ...
    card.addEventListener('dragstart', event => {
      ...
      draggedId = card.dataset.galleryId;

      card.classList.add('gallery-dragging');
...
async function reorderGalleries(draggedId, targetId) {

  const draggedIndex =
    galleriesCache.findIndex(
      g => g.id === draggedId
    );
  ...
  for (const item of updates) {
    ...
    if (error) {
      ...
      flash(
        `Erro ao salvar ordem: ${error.message}`,
        'erro'
      );

      // Recarrega a ordem verdadeira
      await loadGalleries();
...
async function deletePhoto(id){const{data:p,error:q}=await supabase.from('gallery_photos').select('*').eq('id',id).single();if(q||!p){flash('Fotografia não encontrada.','erro');return}if(!confirm('Excluir esta fotografia?'))return;const path=storagePath(p.image_url);if(path){const{error}=await supabase.storage.from(BUCKET).remove([path]);if(error){flash(`Erro ao excluir arquivo: ${error.message}`,'erro');return}}const{error}=await supabase.from('gallery_photos').delete().eq('id',id);if(error){flash(`Erro ao excluir registro: ${error.message}`,'erro');return}if(currentGallery.cover_url===p.image_url){await supabase.from('galleries').update({cover_url:null}).eq('id',currentGallery.id);await ensureCover(currentGallery.id);await refreshGalleryCache(currentGallery.id)}flash('Fotografia excluída.','sucesso');await loadPhotos();await loadGalleries();await loadDashboard()}
async function refreshGalleryCache(id){const{data}=await supabase.from('galleries').select('*, categories(name)').eq('id',id).single();if(data){galleriesCache=galleriesCache.map(g=>g.id===id?data:g);currentGallery=data}}
function storagePath(url){const marker=`/storage/v1/object/public/${BUCKET}/`,i=(url||'').indexOf(marker);return i===-1?null:decodeURIComponent(url.slice(i+marker.length))}
async function loadSettings(){const{data,error}=await supabase.from('site_settings').select('*').limit(1).maybeSingle();if(error){flash(`Erro ao carregar configurações: ${error.message}`,'erro');return}if(!data)return;$('settings-site-name').value=data.site_name||'';$('settings-email').value=data.email||'';$('settings-whatsapp').value=data.whatsapp||'';$('settings-instagram').value=data.instagram_url||'';$('settings-location').value=data.location||'';$('settings-specialty').value=data.specialty||'';$('settings-availability').value=data.availability||'';$('settings-footer').value=data.footer_text||''}
async function saveSettings(e){e.preventDefault();const p={site_name:$('settings-site-name').value.trim(),email:$('settings-email').value.trim()||null,whatsapp:$('settings-whatsapp').value.trim()||null,instagram_url:$('settings-instagram').value.trim()||null,location:$('settings-location').value.trim()||null,specialty:$('settings-specialty').value.trim()||null,availability:$('settings-availability').value.trim()||null,footer_text:$('settings-footer').value.trim()||null,updated_at:new Date().toISOString()};const{data:ex}=await supabase.from('site_settings').select('id').limit(1).maybeSingle();const r=ex?.id?await supabase.from('site_settings').update(p).eq('id',ex.id):await supabase.from('site_settings').insert(p);if(r.error){msg($('settings-msg'),r.error.message,'erro');return}msg($('settings-msg'),'Configurações salvas.','sucesso')}
const esc=v=>String(v??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#039;'),attr=esc;
supabase.auth.onAuthStateChange((_e,s)=>{if(!s){$('login-screen').hidden=false;$('app').hidden=true}});requireAdmin();
